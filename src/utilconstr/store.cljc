(ns utilconstr.store
  "SSoT for the utility-line-construction-coordination actor, behind a
  ...
  (test/utilconstr/store_contract_test.clj), which is the whole point: the
  actor, the Utility-Construction Governor and the audit ledger never know
  ...
  `utilconstr.governor` ns docstring). The site's own `:site-verified?` /
  `:utility-locate-completed?` / `:safety-concern-unresolved?` /
  `:notification-lead-hours-actual` ground-truth fields are what the
  Utility-Construction Governor independently re-checks before a
  `:schedule-construction-operation` proposal may ever commit (see
  `utilconstr.governor`).
  ...
  (:require [utilconstr.registry :as registry]
...
(defprotocol Store
  ...
  (all-sites [s])
  (ledger [s])
  (site-record-log-history [s] "the append-only site-record-log history (utilconstr.registry drafts)")
...
(defn demo-data
  ...
  []
  {:sites
   {"site-1" {:id "site-1" :name "Sakura District Water-Main Renewal"
              ...
              :safety-contacts [{:name "Tanaka (site supervisor)" :email "tanaka@example.com" :phone "+819000000001"}
                                {:name "Suzuki (licensed utility engineer)" :email "suzuki@example.com" :phone "+819000000002"}]
              :status :ready-to-schedule}
    "site-2" {:id "site-2" :name "Atlantis Fiber Backbone Extension"
              :jurisdiction "ATL" :site-verified? true :utility-locate-completed? true
              :safety-concern-unresolved? false
              :notification-lead-hours-actual 200
              :safety-contacts []
              :status :intake}
    "site-3" {:id "site-3" :name "鈴木地区 下水道管延伸工事"
              ...
              :status :unverified}
    "site-4" {:id "site-4" :name "田中地区 ガス管タイイン工事"
    ...
    "site-5" {:id "site-5" :name "Liberty Ave Electric Duct-Bank Upgrade"
              :jurisdiction "JPN" :site-verified? true :utility-locate-completed? true
              :safety-concern-unresolved? false
    ...
    "site-6" {:id "site-6" :name "ことぶき小学校前 水道引込工事"
              :jurisdiction "JPN" :site-verified? true :utility-locate-completed? true
              ...
              :safety-contacts [{:name "Watanabe (site supervisor)" :email "watanabe@example.com" :phone "+819000000005"}
                                {:name "Yamamoto (licensed utility engineer)" :email "yamamoto@example.com" :phone "+819000000006"}]
              :status :concern-open}
    "site-7" {:id "site-7" :name "Riverside Fiber-to-the-Home Trunk"
              ...
              :status :ready-to-schedule}
    "site-8" {:id "site-8" :name "Rheinstraße Gas Distribution Renewal"
...
(def ^:private site-spec
  ...
  {:id                             {:attr :site/id}
   ...
   :trenching-percent-complete     {:attr :site/trenching-percent-complete}
