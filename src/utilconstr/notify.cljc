(ns utilconstr.notify
  ...
  utility-line-construction domain analog of `construction.notify`/
  `demolition.notify`/`roadrail.notify`. Only this namespace touches the
  network (java.net.http, Resend, Twilio) or the RESEND_API_KEY/TWILIO_*
  env vars; every other namespace in this actor stays pure per its own
  contract.
  ...
  `:safety-contacts` roster (the licensed utility engineer / site
  supervisor / utility authority, NOT construction workers -- this actor
  ...
  UNLIKE `construction.notify/dispatch-alert!`, this is NEVER fired by an
  auto-commit -- `:flag-safety-concern` always escalates to a human first
  (see `utilconstr.phase`/`utilconstr.governor` ns docstrings), so by the
  time `utilconstr.operation`'s `:commit` node calls this, a human has
...
(defn dispatch-safety-concern-notice!
  ...
  every other responsible party of a flagged utility-strike/excavation-
  collapse/gas-leak concern."
