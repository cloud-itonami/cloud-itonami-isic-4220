(ns utilconstr.operation
  "OperationActor -- one utility-line-construction-project coordination
  operation = one supervised actor run, expressed as a langgraph-clj
  StateGraph. The advisor (Utility-Construction Advisor) is sealed into a
  single node (:advise); its proposal is ALWAYS routed through the
  Utility-Construction Governor (:govern) and the rollout phase gate
  ...
  One graph run = one utility-line-construction coordination operation
  ...
  decision to a human operator (the licensed utility engineer / site
  supervisor). The approver resumes with `{:approval {:status :approved}}`
  (or :rejected). `:schedule-construction-operation`/`:flag-safety-
  concern` ALWAYS reach this node -- see `utilconstr.phase`/`utilconstr.
  ...
  is clean -- see `utilconstr.phase` ns docstring.

  Every committed record carries `:effect :propose` (see `utilconstr.
  ...
  ADDITIONALLY send the safety-concern notice (mail + phone, `utilconstr.
  notify`) to the site's `:safety-contacts` roster -- always AFTER a
  human has approved it, since `:flag-safety-concern` is never auto-
  eligible at any phase."
...
(defn build
  ...
  `utilconstr.store/Store`).
  opts:
    :advisor      -- a `utilconstr.advisor/Advisor` (default: mock-advisor)
    :notifier     -- a `utilconstr.notify/Notifier` (default: mock-notifier)
  ...
  (-> (g/state-graph
      ...
      ;; Utility-Construction Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Utility-Construction Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
