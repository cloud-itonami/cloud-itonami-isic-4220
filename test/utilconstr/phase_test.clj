(ns utilconstr.phase-test
  "The phase table as executable tests. The invariant this repo cannot
  regress on: `:schedule-construction-operation`/`:flag-safety-concern`
  must NEVER be a member of any phase's `:auto` set. `:log-site-record`
  and `:order-supplies` ARE auto-eligible at phase 3 -- see `utilconstr.
  phase` ns docstring 'Actuation' section."
  (:require [clojure.test :refer [deftest is testing]]
            [utilconstr.phase :as phase]))

(deftest schedule-construction-operation-never-auto-at-any-phase
  (testing "structural invariant: no phase auto-commits a construction-operation schedule proposal"
    (doseq [[n {:keys [auto]}] phase/phases]
      (is (not (contains? auto :schedule-construction-operation))
          (str "phase " n " must not auto-commit :schedule-construction-operation")))))
...
(deftest write-ops-is-exactly-the-closed-four-op-allowlist
  (is (= #{:log-site-record :schedule-construction-operation :flag-safety-concern :order-supplies}
         phase/write-ops)))
...
(deftest gate-escalates-a-clean-non-auto-write
  (is (= :escalate (:disposition (phase/gate 3 {:op :schedule-construction-operation} :commit))))
