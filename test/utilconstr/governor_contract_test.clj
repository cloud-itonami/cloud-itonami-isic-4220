(ns utilconstr.governor-contract-test
  "The governor contract as executable tests -- the utility-line-
  construction-coordination analog of `construction.governor-contract-
  test`/`demolition.governor-contract-test`/`roadrail.governor-contract-
  test`. The single invariant under test:

    Utility-Construction Advisor never schedules a construction operation,
    files a safety-concern flag or a supply order the Utility-Construction
    Governor would reject; `:schedule-construction-operation`/`:flag-
    safety-concern` NEVER auto-commit at any phase; `:log-site-record` (no
    direct capital/safety risk) MAY auto-commit when clean; `:order-
    supplies` MAY auto-commit when clean AND below the cost threshold; and
    every decision (commit OR hold) leaves exactly one ledger fact. Every
...
(deftest clean-log-site-record-auto-commits
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :log-site-record :subject "site-1" :patch {:id "site-1" :trenching-percent-complete 60}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= 60 (:trenching-percent-complete (store/site db "site-1"))) "SSoT actually updated")
...
(deftest fabricated-jurisdiction-is-held
  (testing "site-2 (ATL, no spec-basis in utilconstr.facts) -> HOLD, never reaches a human"
    (let [[db actor] (fresh)
...
(deftest flag-safety-concern-triggers-notification-only-after-approval
  (let [[_db actor] (fresh)
        r1 (exec-op actor "t11" {:op :flag-safety-concern :subject "site-1"
                                 :concern-type :excavation-collapse :concern-description "trench wall sloughing observed"} operator)]
...
(deftest order-supplies-below-threshold-auto-commits
  (let [[db actor] (fresh)
        res (exec-op actor "t12" {:op :order-supplies :subject "site-1"
                                  :items ["HDPE-pipe-150mm"] :cost-usd 1200 :vendor "Local Utility Supply Co."} operator)]
    (is (= :commit (get-in res [:state :disposition])))
...
(deftest forbidden-action-class-markers-are-permanent-hard-violations
  (doseq [marker [:equipment-control? :direct-actuation? :finalizes-tie-in-authorization? :finalizes-energization-authorization?]]
    (testing marker
      (let [db (store/seed-db)
...
(deftest every-decision-leaves-one-ledger-fact
  (testing "write-only-through-ledger: N operations -> N ledger facts"
    (let [[db actor] (fresh)]
      (exec-op actor "a" {:op :log-site-record :subject "site-1" :patch {:id "site-1" :trenching-percent-complete 10}} operator)
