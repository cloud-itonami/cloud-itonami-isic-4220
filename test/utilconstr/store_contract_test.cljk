(ns utilconstr.store-contract-test
  ...
  a configuration change, not a rewrite -- see `construction.store-
  contract-test`/`demolition.store-contract-test`/`roadrail.store-
...
(deftest read-parity
  (doseq [[label s] (backends)]
    (testing label
      (is (= "Sakura District Water-Main Renewal" (:name (store/site s "site-1"))))
      ...
      (is (= 200 (:notification-lead-hours-actual (store/site s "site-1"))))
      (is (false? (:site-verified? (store/site s "site-3"))) "site-3 seeded not-yet-verified")
      (is (false? (:utility-locate-completed? (store/site s "site-4"))) "site-4 seeded with an incomplete utility locate")
      (is (= 20 (:notification-lead-hours-actual (store/site s "site-5"))) "site-5 seeded with an insufficient JPN lead time")
    ...
    (testing label
      (testing "log-site-record commits a patch and appends a site-record-log entry"
        (store/commit-record! s {:op :log-site-record :path ["site-1"]
                                 :value {:id "site-1" :trenching-percent-complete 75}})
        (is (= 75 (:trenching-percent-complete (store/site s "site-1"))))
        (is (= "Sakura District Water-Main Renewal" (:name (store/site s "site-1"))) "unrelated field preserved")
        (is (= "JPN-SRL-000000" (get (first (store/site-record-log-history s)) "record_id")))
        (is (= 1 (store/next-site-record-sequence s "JPN"))))
      (testing "schedule-construction-operation drafts a record and advances the sequence"
        (store/commit-record! s {:op :schedule-construction-operation :path ["site-1"]
      ...
      (testing "flag-safety-concern sets safety-concern-unresolved? true, drafts a record + notice document"
        (store/commit-record! s {:op :flag-safety-concern :path ["site-1"]
                                 :value {:site-id "site-1" :concern-description "test unmarked line"}})
      ...
      (testing "order-supplies drafts a record and advances the sequence"
        (store/commit-record! s {:op :order-supplies :path ["site-1"]
                                 :value {:site-id "site-1" :items ["pipe-fitting"] :cost-usd 500}})
      ...
      (testing "sequences are jurisdiction-scoped, not global"
        (store/commit-record! s {:op :log-site-record :path ["site-7"]
                                 :value {:id "site-7" :trenching-percent-complete 10}})
...
(deftest datomic-empty-store-is-usable
  (let [s (store/datomic-store)]
    ...
    (store/with-sites s {"x" {:id "x" :name "n" :jurisdiction "JPN"
                              ...
                              :safety-concern-unresolved? false :notification-lead-hours-actual 200
                              :status :intake}})
