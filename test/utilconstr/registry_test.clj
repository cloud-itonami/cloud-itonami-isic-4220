(ns utilconstr.registry-test
  (:require [clojure.test :refer [deftest is]]
            [utilconstr.registry :as r]))
...
(def sample-site
  {:id "site-1" :name "Sakura District Water-Main Renewal" :jurisdiction "JPN"})

(deftest safety-concern-notice-cites-legal-basis-inline
  (let [doc (r/render-safety-concern-notice sample-site "JPN-SCF-000000" "試掘溝で未表示のガス管を確認")]
    (is (re-find #"JPN-SCF-000000" doc))
    (is (re-find #"労働安全衛生規則" doc))
    (is (re-find #"laws\.e-gov\.go\.jp" doc))
    (is (re-find #"未表示のガス管" doc))
    (is (re-find #"licensed utility engineer" doc) "honestly disclaims tie-in/energization-authorization authority")))
